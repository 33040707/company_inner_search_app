...
def display_initial_ai_message():
    ...
    AIメッセージの初期表示（サイドバー用）
    """
    # 【変更】with st.chat_message は削除し、直接サイドバーへ表示
    st.markdown("こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。")
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']}(Page: {message['content']['main_page_number']})", icon=icon)
                        else:
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                ...
                # 「ファイルパス」のみ
                file_info = f"{file_path}(Page: {page_number})"
